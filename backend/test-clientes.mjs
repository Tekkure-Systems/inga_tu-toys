...
async function main() {
  ...
  try {
    ...
    try {
      const [countRows] = await conn.query('SELECT COUNT(*) AS cnt FROM cliente');
      console.log('cliente table row count:', countRows[0].cnt);

      const [sample] = await conn.query('SELECT id_cliente, nombre, correo, domicilio FROM cliente ORDER BY id_cliente DESC LIMIT 5');
