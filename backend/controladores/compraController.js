        ...
        db.query('START TRANSACTION', (err) => {
            ...
            db.query(insertCompraSql, [dirId, cliente], (err, result) => {
                ...
                db.query(insertPedidoSql, [values], (err) => {
                    ...
                    let updateSql = 'UPDATE producto SET cantidad = CASE';
                    const productIds = [];

                    for (const [productoId, cantidadComprada] of productosComprados.entries()) {
                        if (!productoId) {
                            return db.query('ROLLBACK', () => callback(new Error('productoId undefined al construir UPDATE')));
                        }
                        updateSql += ` WHEN id_producto = ${db.escape(productoId)} THEN cantidad - ${db.escape(cantidadComprada)}`;
                        productIds.push(db.escape(productoId));
                    }

                    updateSql += ' ELSE cantidad END';
                    updateSql += ` WHERE id_producto IN (${productIds.join(',')})`;
...
export const capturePayPalOrder = async (req, res) => {
    ...
    try {
        ...
        if (!isCompleted || !hasCliente || !hasItems) {
            ...
            // Si no se cumplen, manda la respuesta de error y TERMINA aquí
